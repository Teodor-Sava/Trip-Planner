            ...
            City.find({
                ...
                .then(cities => {
                    if (cities.length > 0) {
                        ...
                        cities.forEach(city => {
                            const radius = 1500;
                            const googlePlacesURL = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=' +
                            ...
                            request.get(googlePlacesURL, {
                                json: true
                            }, (err, response, body) => {
                                if (body.status === 'OK') {
                                    const places = body.results;
                                    const path_to_error_file = `${appDir}/cron_reports/places_report_${moment().format('DD-MM-YYYY')}.txt`;
                                    ...
                                    places.forEach(async (placeAPI) => {
                                        let tags;
                                        tags = await Promise.all(placeAPI.types.map(async (tagAPI) => {
                                        ...
                                        await Place.findOne({
                                            ...
                                            .catch(err => writeToFile(path_to_error_file,
                                                `place: ${place.name} name: ${city.name} error : ${err.message}`));

                                    });

                                    res.status(200).json({
                                        message: `Number of places migrated ${numberOfPlacesMigrated}/${places.length}`
                                    })
                                }
                            })
                        })
